...
void mx6_cpu_regulator_init(void)
{
	...
	*/
	...
	printk(KERN_INFO "cpu regulator (%s) mode:%s\n", gp_reg_id, (enable_ldo_mode ==
...
#endif
	{
		...
		if (IS_ERR(cpu_clk)) {
			...
			if (IS_ERR(soc_regulator))
				...
				printk("%s: set %s regulator to %d uV.\n",__func__, soc_reg_id, cpu_op_tbl[0].soc_voltage);
			...
			if (!IS_ERR(cpu_regulator)) {
				...
				printk("%s: set %s regulator to %d uV.\n",__func__, gp_reg_id, cpu_op_tbl[0].cpu_voltage);
