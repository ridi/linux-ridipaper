...
static enum usb_wakeup_event _is_host_wakeup(struct fsl_usb2_platform_data *pdata)
{
	...
	if (dr_utmi_config.id_gpio) {
		printk ("[%s-%d] g_usb_id %d\n", __func__, __LINE__, g_usb_id);
		if(g_usb_id) {
...
static enum usb_wakeup_event _is_device_wakeup(struct fsl_usb2_platform_data *pdata)
{
	...
	if (dr_utmi_config.id_gpio) {
		printk ("[%s-%d] gpio %d, id %d\n", __func__, __LINE__, dr_utmi_config.id_gpio, g_usb_id);
	/* if ID=1, it is a device wakeup event */
		if (wakeup_req && (g_usb_id) && (UOG_USBSTS & USBSTS_URI)) {
			printk(KERN_INFO "otg udc wakeup, host sends reset signal\n");
			return WAKEUP_EVENT_DPDM;
		}
		if (wakeup_req && (g_usb_id) &&  \
		...
		if (wakeup_req && (g_usb_id) && (UOG_OTGSC & OTGSC_STS_A_VBUS_VALID) \
...
static int  __init mx6_usb_dr_init(void)
{
	...
	if (dr_utmi_config.id_gpio) {
		gpio_request (dr_utmi_config.id_gpio, "usb_otg_id");
		gpio_direction_input (dr_utmi_config.id_gpio);
    	g_usb_id = gpio_get_value (dr_utmi_config.id_gpio)?0:1;
		printk ("[%s-%d] id_gpio %d, g_usb_id %d\n", __func__, __LINE__, dr_utmi_config.id_gpio, g_usb_id);
		if (!g_usb_id) {
			dr_utmi_config.id_gpio = IMX_GPIO_NR(3, 14); 	// set to always high pad (client)
			gpio_request (dr_utmi_config.id_gpio, "usb_otg_id_client");
			gpio_direction_input (dr_utmi_config.id_gpio);
		}
//		else
//			dr_utmi_config.id_gpio = IMX_GPIO_NR(3, 11);	// set to always low pad (host)
    }
