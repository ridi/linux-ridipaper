				     ...
				     struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_pll(codec_dai, ALC5623_PLL_FR_MCLK,
				  ALC5623_PLL_FR_MCLK, priv->sysclk,
				  pll_out);
	if (ret < 0)
		pr_err("Failed to start FLL: %d\n", ret);

	ret = snd_soc_dai_set_sysclk(codec_dai, ALC5623_PLL_FR_MCLK,
					 card_priv.sysclk,
					 SND_SOC_CLOCK_IN);
	ret = snd_soc_dai_set_sysclk(codec_dai, ALC5623_PLL_FR_MCLK, card_priv.sysclk, 0);
	if (ret < 0) {
...
static const struct snd_soc_dapm_route alc5623_route[] = {
...
static int imx_alc5623_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_add_routes(dapm, alc5623_route, ARRAY_SIZE(alc5623_route));
...
static struct snd_soc_dai_link imx_dai[] = {
	{
		...
		.codec_name	= "alc562x-codec.2-001a",
		.cpu_dai_name	= "imx-ssi.1",
		.platform_name	= "imx-pcm-audio.1",
		.init		= imx_alc5623_init,
		.ops		= &imx_hifi_ops,
	},
};
 ...
 */
static int __devinit imx_alc5623_probe(struct platform_device *pdev)
{
...
static int __devexit imx_alc5623_remove(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct imx_priv *priv = &card_priv;
...
static struct platform_driver imx_alc5623_driver = {
	.probe = imx_alc5623_probe,
	.remove = imx_alc5623_remove,
	.driver = {
		   .name = "imx-alc5623",
		   .owner = THIS_MODULE,
		   },
...
static int __init imx_asoc_init(void)
{
	...
	ret = platform_driver_register(&imx_alc5623_driver);
	if (ret < 0)
		goto exit;
...
err_device_alloc:
	platform_driver_unregister(&imx_alc5623_driver);
exit:
	return ret;
}
...
/* Module information */
MODULE_DESCRIPTION("ALSA SoC imx alc5623");
MODULE_LICENSE("GPL");
