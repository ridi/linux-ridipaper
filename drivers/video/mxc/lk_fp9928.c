...
#define FP9928_POWEROFF_ms_MAX			0 //
#define FP9928_PWROFFDELAYWORK_ms		500
		...
		if(iIsEnable) {
...
#endif //]RAILPWR_SCHED_WAIT==1
			{
				...
				while(!gpio_get_value(GPIO_FP9928_PWRGOOD)) {
					dwTestCnt++;
					if(time_after(jiffies,dwTickTimesout)) {
			...
			gpio_direction_output(GPIO_FP9928_EN,EN_OFF);
			gptFP9928_data->iIsOutputEnabled = 0;
			gptFP9928_data->dwTickPowerOffEnd = jiffies + msecs_to_jiffies(FP9928_POWEROFF_ms_MAX);
		}
	}
	...
	if (iIsPowerOn) {
		...
		if(!gptFP9928_data->iIsOutputPowerDownCounting) 
		{
			...
			schedule_delayed_work(&gptFP9928_data->tPwrdwn_work_param.pwrdwn_work, \
					msecs_to_jiffies(FP9928_PWROFFDELAYWORK_ms));
		}
	}
	mutex_unlock(&gptFP9928_data->tPwrLock);	
...
void fp9928_shutdown(void)
{
	...
	msleep(FP9928_PWROFFDELAYWORK_ms);
	//DBG0_ENTRY_TAG();
	_fp9928_output_en(0);
	//DBG0_ENTRY_TAG();
	_fp9928_vin_onoff(0);
