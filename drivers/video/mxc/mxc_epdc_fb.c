...
#define TCE_UNDERRUN_PREVENT_MCORE_PATCH	1
			       ...
			       bool use_test_mode, u32 np_val)
{
	...
	if(giLast_waveform_mode!=waveform_mode) {
		if( 9==gptHWCFG->m_val.bCustomer && 
				g_fb_data->wv_modes.mode_a2==waveform_mode &&
...
int mxc_epdc_wait_for_frameend(struct fb_info *info) {
    ...
    int ret = 0;
    DBG_MSG("\e[35m[%s %d]&fb_data->before_eof_event:%x\e[0m\n",__func__, __LINE__,&fb_data->before_eof_event);

	ret = wait_for_completion_timeout(&fb_data->before_eof_event,
...
static void epdc_submit_work_func(struct work_struct *work)
{
	...
	if(1!=fb_data->tce_underrun_occurred) {
		fb_data->tce_underrun_occurred=0;
	}
				   ...
				   struct fb_info *info)
{
	...
	EPDC_VPRINT(fb_data,1,"\e[32m%s():upd update mode=%s,wf mode=%d,left=%d,top=%d,w=%d,h=%d,flag=0x%x,marker=%d\e[0m\n",
			__FUNCTION__,
...
static irqreturn_t mxc_epdc_irq_handler(int irq, void *dev_id)
{
	...
	DBG_MSG("\e[36m[%s %d] ints_fired:%x luts1_ints_fired:%x luts2_ints_fired:%x\e[0m\n",__func__, __LINE__,ints_fired,luts1_ints_fired,luts2_ints_fired);
	...
	if (epdc_signal_eof()) {
		GALLEN_DBGLOCAL_RUNLOG(7);
		DBG_MSG("\e[33m[%s %d] frame end\e[0m\n",__func__, __LINE__);
		hrtimer_start( &before_eof_hrtimer, before_eof_ktime, HRTIMER_MODE_REL );
...
static void epdc_intr_work_func(struct work_struct *work)
{
	...
	//if(fb_data->tce_underrun_occurred) 
	{
		...
		EPDC_VPRINT(fb_data,10,"wb_busy=0x%x\n",epdc_wb_busy);
		EPDC_VPRINT(fb_data,2,"luts_status=0x%llx\n",epdc_luts_status(fb_data->rev));
		...
		EPDC_VPRINT(fb_data,10,"waiting wb=%d,lut=%d\n",
				fb_data->waiting_for_wb,fb_data->waiting_for_lut);
	}
	...
	if ((!wb_pause && epdc_wb_busy) || (wb_pause && !epdc_upd_done)) {
		...
		mutex_unlock(&fb_data->queue_mutex);
		GALLEN_DBGLOCAL_ESC();
		EPDC_VPRINT(fb_data,10,"%s() exit - wb busy\n",__FUNCTION__);
		return;
	}
	...
	list_for_each_entry(collision_update,
		 ...
		 */
		collision_update->update_desc->upd_data.update_mode=UPDATE_MODE_PARTIAL;
...
int __devinit mxc_epdc_fb_probe(struct platform_device *pdev)
{
	...
	if(1==gptHWCFG->m_val.bDisplayResolution) {
		...
		if(gptHWCFG->m_val.bPCB == 76)
		{
				fb_data->cur_mode->vmode->pixclock=113400000;
				printk("%s(%d):EPD pixclock=%d \n",__FILE__,__LINE__,fb_data->cur_mode->vmode->pixclock);
		}		
	}
	else if(2==gptHWCFG->m_val.bDisplayResolution) {
		printk("%s(%d):EPD 1024x768 \n",__FILE__,__LINE__);
	...
	if (2 == gptHWCFG->m_val.bUIStyle) { //Android
		if ( 5 == gptHWCFG->m_val.bDisplayPanel || 
                 ...
                 29 == gptHWCFG->m_val.bDisplayPanel ) 
